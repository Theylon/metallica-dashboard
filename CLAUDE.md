...
| `scripts/rare_earth_leadlag.py` | Research: does a rare-earth assessment (default PrNd oxide 270930) lead, confirm or contradict MP / REMX / LYSCF? Cross-correlation, trend IC and the divergence grid with a pre-fixed verdict rule → `reports/rare_earth_leadlag.md`. Inputs (dump + equities JSON) are passed by path and never committed. **The publication delay is the whole ball game**: at the assumed 11 trading days the "stock up / PrNd down" cell looked strong on MP (−11.9%, p 0.005), but the first live pull measured the real delay at 27 calendar days (~19 trading), and at 19 only LYSCF survives (−9.5%, p 0.058) while MP falls to −2.0% (p 0.43). The Action passes `--delay` from `data/mm_freshness.json`; never hard-code it. Research only — no gate or score reads this. |
