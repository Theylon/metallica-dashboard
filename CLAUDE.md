...
| `scripts/exposure.py` + `data/linkage_map.json` | Per-holding commodity exposure from the upstream (metallica-fund) signal-mined linkage map. **Index links (MetalMiner MMIs, category `mmi index values`) never count as exposure** — an MMI is a stepped monthly composite, not a price (rare-earth MMI vs PrNd oxide weekly-return correlation ≈ 0.22, vs 0.79–0.97 among the real assessments). `verify_data.py` warns when the map carries T1/T2 index links or a Rare Earth ticker has no real price link. |
| `scripts/mmi_proxy_audit.py` | Re-measures an MMI against the assessments it proxies from a MetalMiner historical dump (path on the command line — licensed data, **never commit the dump or an API token**) → `reports/mmi_proxy_audit.md`. |
| `scripts/mm_fetch.py` + `scripts/mm_equities.py` | Daily MetalMiner pull in the fetch Action: `mm_fetch.py` reads `METALMINER_API_TOKEN` (Actions secret, **never in git**), writes a dump-shaped file to `/tmp` for the scripts below and `data/mm_freshness.json` (per-series last date + lag days — the real publication delay). `mm_equities.py` pulls MP/REMX/LYSCF closes from Yahoo to `/tmp` for `rare_earth_leadlag.py`. Both best-effort; the Action deletes the `/tmp` files afterwards. |
| `scripts/mm_series_quality.py` | Grades every series in a MetalMiner dump **tradable / level_only / rejected** (stale, frozen, composite index, unit flip, cadence) → `reports/mm_series_quality.md` + `data/mm_series_registry.json`. Run it on every new dump; `verify_data.py` warns when a T1/T2 link sits on a rejected series. Thresholds are constants at the top of the script. |
| `scripts/rare_earth_leadlag.py` | Research: does a rare-earth assessment (default PrNd oxide 270930) lead, confirm or contradict MP / REMX / LYSCF? Cross-correlation, trend IC and the divergence grid with a pre-fixed verdict rule → `reports/rare_earth_leadlag.md`. Inputs (dump + equities JSON) are passed by path and never committed. **The publication delay decides the answer, so measure it on a healthy feed**: at the true lag (4 calendar days ≈ 3 trading) "stock up 20d / PrNd down 20d" gives MP −16.1% and LYSCF −11.2% fwd 20d (both p<0.001), REMX not confirmed. A delay measured during the 2026-08-07→09-04 outage read 27 days and wrongly collapsed the result — hence `MAX_SANE_DELAY` and the Action skipping the run while 270930 is graded rejected. Research only — no gate or score reads it. |
