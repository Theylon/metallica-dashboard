...
Every card also carries an **AI Hedge Fund** multi-analyst verdict — the concept ported from [virattt/ai-hedge-fund](https://github.com/virattt/ai-hedge-fund) (MIT; no upstream code is copied). A roster of lenses scores each name: four **analytical** agents (Valuation, Fundamentals, Technicals, Sentiment), a **Risk Manager**, and a **Portfolio Manager** whose call becomes the aggregate verdict (signal + conviction + suggested action, e.g. a bullish read on a held short → *cover*). Each lens emits bullish/bearish/neutral + a 0-100 confidence + one line of reasoning.

The layer is **display-only** — it does not feed the composite score. The lenses are deliberate tilts over signals already in the composite (momentum, analyst, fundamentals, commodity, sentiment), so weighting them would double-count; keeping it display-only also means the free 4×/day price refresh never has to reconcile it. A daily Claude Routine refreshes the verdicts (research shards), and a deterministic Python fallback (`scripts/gen_hedge_auto.py`) guarantees all 204 names always carry a valid verdict even with no live run — the same backstop pattern the deep dives use.
