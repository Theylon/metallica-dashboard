...
Each ticker's card now carries five data layers: **market snapshot**, **live IBKR position**, **recommendation + justification**, **fundamentals** (TrueNorth FY metrics — profitability, balance sheet, growth for 46 held/recommended/large-cap names), **underlying-asset exposure** (OM linkage 0-9 broken into revenue-exposure / price-sensitivity / coupling, each 0-3, with evidence), plus the commodity backdrop, thesis, catalysts, risks and sourced evidence. **Every one of the ~190 scoreable names now carries an evidence-based deep-dive from Bigdata.com** (news / filings / transcripts, cited with source + date) — ~50 hand-researched in the focus set, ~140 researched in a batched Bigdata sweep. The earlier model-derived placeholders have been replaced. Untradable listings (Norilsk/Moscow, Lima-only, sanctioned) are score-capped and flagged. Conflicting evidence is surfaced, not hidden — e.g. Berenberg's Buy upgrade on Anglo American Platinum is flagged against the PGM-short bias; Centerra's +160% YoY earnings noted against its short-lean.
